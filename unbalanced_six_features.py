...
def do_predictions(X_train, X_test, y_train, y_test, i, params):
    ...
    # getting feature importances to see wether the used features seem sensible
...
def pretrain (X_test,y_test,X_train, y_train, s_X, s_y, n_pre, n_real, i, params_pre, params_fine, RL=False): 
    "Conducts the pretraining, creates two random forests and combines their votes into a final prediction"
...
plt.title('Base simulated data')
plt.xticks(range(len(OL_accuracys)), ["0.2","0.5","1"])
plt.grid(True)
legend_entries = [plt.Line2D([0], [0], color='green', lw=2, label='$\it{Literature}$'),plt.Line2D([0], [0], color='indianred', lw=2, label='$\it{Real + Literature}$'),plt.Line2D([0], [0], color='black', lw=2, label=r'$\it{Standard}$' )]
...
results = pd.DataFrame({
    ...
    "std spez": [specificity_std, specificity_std_OL_20, specificity_std_OL_50, specificity_std_OL_100, specificity_std_RL_20, specificity_std_RL_50, specificity_std_OL_100 ],
